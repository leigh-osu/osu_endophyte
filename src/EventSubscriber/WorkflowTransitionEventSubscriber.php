...
namespace Drupal\osu_endophyte\EventSubscriber;

use Drupal\Core\Entity\ContentEntityInterface;
use Drupal\Core\Entity\EntityPublishedInterface;
use Drupal\Core\Entity\RevisionableInterface;
use Drupal\osu_endophyte\WorkflowHelperInterface;
...
class WorkflowTransitionEventSubscriber implements EventSubscriberInterface {
   ...
   * The workflow helper.
   *
   * @var \Drupal\osu_endophyte\WorkflowHelperInterface
   */
   ...
   *
   * @param \Drupal\osu_endophyte\WorkflowHelperInterface $workflowHelper
   *   The workflow helper.
   */
...
//    $is_published_state = $this->isPublishedState($event->getTransition()->getToState(), $event->getWorkflow());
    ...
    $is_notify_client_state = $this->isNotifyClientState($event->getTransition()->getToState(), $event->getWorkflow());

    $fields = $this->workflowHelper->getEntityStateField($entity);

    if ($entity instanceof EntityPublishedInterface) {
      if ($is_notify_client_state) {
//        \Drupal::messenger()->addMessage(t('Send an email!'));
      }
      else {
//        $entity->setUnpublished();
        \Drupal::messenger()->addMessage(t('No email!'));
      }
   ...
   *   TRUE if the state is set as published in the workflow, FALSE otherwise.
   */
  protected function isNotifyClientState(WorkflowState $state, WorkflowInterface $workflow) {
    return $this->workflowHelper->isWorkflowStateNotifyClient($state->getId(), $workflow);
  }
