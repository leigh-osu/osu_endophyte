...
class WorkflowHelper implements WorkflowHelperInterface {
   ...
   * {@inheritdoc}
   */
  public function isWorkflowStateNotifyClient($state_id, WorkflowInterface $workflow) {
    // We rely on being able to inspect the plugin definition. Throw an error if
    // this is not the case.
    if (!$workflow instanceof PluginInspectionInterface) {
      $label = $workflow->getLabel();
    ...
    // are stored there.
    $raw_workflow_definition = $workflow->getPluginDefinition();
    $field_name = !empty($raw_workflow_definition['states'][$state_id]['notify_client']) ? $raw_workflow_definition['states'][$state_id]['notify_client'] : FALSE;
    return $field_name;
  }
